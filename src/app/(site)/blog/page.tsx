...
type Item = any; // คงโครงเดิมทุกฟิลด์ ไม่บังคับ type เพื่อลดการตัดฟิลด์โดยไม่ตั้งใจ
...
export default function BlogPage(){
  // อย่า clone / map item; ใช้อ้างอิงตรง ๆ
  const items: Item[] = BLOG_ITEMS as Item[]
  ...
  // คำนวณ “ดัชนีที่ผ่านเงื่อนไข” เพื่อคงอ้างอิง item เดิม 100%
  const filteredIdx = useMemo(() => {
    const idxs = items.map((_, i) => i)

    // search (ไม่แก้ไข item)
    const qq = q.trim().toLowerCase()
    let after = idxs
    if (qq) {
      after = after.filter(i => {
        const it = items[i]
        // รวมทุกฟิลด์เป็นข้อความเพื่อค้นหา “แต่ไม่แก้ค่าใน item”
        const hay = [
          it?.title, it?.excerpt, it?.content, it?.body, it?.description,
          it?.details, it?.longContent, it?.fullText,
          // ตัด tag HTML ออกจาก contentHtml แค่สำหรับค้นหา
    ...
    // ปี
    if (year !== 'all') {
      after = after.filter(i => getYear(items[i]?.date) === year)
    }

    // tag
    if (tag !== 'all') {
      after = after.filter(i => Array.isArray(items[i]?.tags) && items[i].tags.includes(tag))
    }

    // sort โดยอ้างอิงดัชนี
  ...
  // แปลงดัชนีกลับเป็นอาร์เรย์ของ item (อ้างอิงเดิม)
    ...
    <div className="relative space-y-8">
      ...
      <section className="relative rounded-3xl glass-deep ring-1 ring-white/40 p-6 md:p-8">
        <div className="flex flex-wrap items-start gap-4">
          <div className="grow">
            <h1 className="text-3xl md:text-4xl font-bold tracking-tight">บทความกฎหมาย</h1>
            <p className="mt-3 max-w-3xl text-slate-700">
              รวบรวมบทความสรุปประเด็นกฎหมายสำคัญ อ่านง่าย ใช้ภาษาชัดเจน
            </p>
            <div className="mt-4 flex flex-wrap gap-2 text-sm">
              ...
              <span className="inline-flex items-center gap-2 rounded-xl bg-white/80 px-3 py-1 ring-1 ring-slate-200">
                ปี {thisYear}: {totalThisYear} บทความ
              </span>
            </div>
          </div>
          ...
          <label className="group grid">
            ...
            <div className="flex items-center gap-2 rounded-xl ring-1 ring-slate-200 bg-white px-3 py-2">
              ...
              <input
                ...
                placeholder="ผู้บริโภค, อาญา, สัญญา ..."
      ...
      {/* GRID */}
      <ItemsGrid items={filteredItems} kind="blog" />
    </div>
  )
}
